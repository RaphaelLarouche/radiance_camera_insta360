...
from source.radiance import attenuation_coefficient, RadClass
...
def graph_dort_vs_measurements(depths, irradiances, irradiance_labels):
    ...
    #pcolor = ["#a6cee3", "#1f78b4", "#b2df8a"]
    ncolor = ["#1f78b4", "#b2df8a", "#e31a1c"]
    ...
    for i, a in enumerate(irradiances):
        ...
        for b in range(3):
            ...
            #ax[b].plot(i_d[band_name[b]], depths, marker=mstyle[i], markersize=2, linewidth=0.8, color="#1f78b4", linestyle=lstyle[i], label="$E_{d}$")
            #ax[b].plot(i_u[band_name[b]], depths,  marker=mstyle[i], markersize=2, linewidth=0.8, color="#b2df8a", linestyle=lstyle[i], label="$E_{u}$")
            #ax[b].plot(i_o[band_name[b]], depths,  marker=mstyle[i], markersize=2, linewidth=0.8, color="#e31a1c", linestyle=lstyle[i], label="$E_{0}$")

            ax[b].plot(i_d[band_name[b]], depths, linewidth=0.8, color="#1f78b4", linestyle=lstyle[i], label="$E_{d}$")
            ax[b].plot(i_u[band_name[b]], depths, linewidth=0.8, color="#b2df8a", linestyle=lstyle[i], label="$E_{u}$")
            ax[b].plot(i_o[band_name[b]], depths, linewidth=0.8, color="#e31a1c", linestyle=lstyle[i], label="$E_{0}$")
...
def open_TriOS_data(min_date="2018-08-31 8:00", max_date="2018-08-31 16:00", path_trios="data/2018R4_300025060010720.mat"):
    ...
    #albedo = radiometer["albedo_i1r1"][inx].T / 1000

    return timestamps, radiometer["wl"], irradiance_incom, irradiance_below


def open_albedo_transmittance_meiralportal(min_date="2018-08-31 8:00", max_date="2018-08-31 16:00",
                                           genpath="data/meiral_portal_2018R4/"):

    # Albedo
    ramses_albedo = pandas.read_csv(genpath + "2018R4_300025060010720_RAMSES_albedo.csv")
    dt_a = pandas.to_datetime(ramses_albedo["Date/Time"])
    mask = (dt_a <= max_date) & (dt_a >= min_date)
    inx = ramses_albedo["Date/Time"][mask].index

    ramses_albedo_clean = ramses_albedo.iloc[inx]
    dt_albedo_clean = ramses_albedo_clean["Date/Time"].to_numpy(dtype="datetime64")

    # Transmittance
    ramses_transmittance = pandas.read_csv(genpath + "2018R4_300025060010720_RAMSES_transmittance.csv")
    dt_t = pandas.to_datetime(ramses_transmittance["Date/Time"])

    mask_t = (dt_t <= max_date) & (dt_t >= min_date)
    inx_t = ramses_transmittance["Date/Time"][mask_t].index
    print(inx_t)

    ramses_tr_clean = ramses_transmittance.iloc[inx_t]
    dt_tr_clean = ramses_tr_clean["Date/Time"].to_numpy(dtype="datetime64")

    # Wavelength
    wl = np.arange(320, 951, 1)

    return wl, dt_albedo_clean, ramses_albedo_clean.to_numpy()[:, 6:], dt_tr_clean, ramses_tr_clean.to_numpy()[:, 6:]
...
def graph_cam_vs_simulations(radiance_camera, zenith_camera, radiance_simul, zenith_simul, cam_keys_order, simul_keys_order):
    ...
    all_rad_cam = np.zeros((len(simul_keys_order), 181, 3))
    all_rad_sim = np.zeros((len(simul_keys_order), 181, 3))
    ...
    for i, (cam_k, simul_k) in enumerate(zip(cam_keys_order, simul_keys_order)):
        ...
        for b in range(rad_cam.shape[2]):
            ...
            # RMS
            all_rad_cam[i, :, b] = rad_cam_avg[:, b]
            all_rad_sim[i, :, b] = rad_sim_avg_interpo
    ...
    rms_tot = rad_rmse(all_rad_cam, all_rad_sim)

    ax[2, 0].set_xlabel("Zenith [˚]")
    ax[2, 1].set_xlabel("Zenith [˚]")
    ax[2, 2].set_xlabel("Zenith [˚]")

    ax[0, 2].text(25, np.round(np.max(mre_profile * 100)) + 8, "MUAPD = {0:.2f} %\nRMSE = {1:.2f}%".format(mupd[0], rms_tot[0]), fontsize=6)
    ax[1, 2].text(25, np.round(np.max(mre_profile * 100)) + 8, "MUAPD = {0:.2f} %\nRMSE = {1:.2f}%".format(mupd[1], rms_tot[1]), fontsize=6)
    ax[2, 2].text(25, np.round(np.max(mre_profile * 100)) + 8, "MUAPD = {0:.2f} %\nRMSE = {1:.2f}%".format(mupd[2], rms_tot[2]), fontsize=6)
...
if __name__ == "__main__":
    ...
    rc.show_smoothed_radiance_curves(raw=False)
    #rc.show_irradiance_curves()
    #rc.show_mean_cosines()
    #rc.show_absorption_coefficient()


    # Open TrioS data
    time_stamp, trios_wl, ed_trios_incom, ed_trios_below = open_TriOS_data(min_date="2018-08-31 8:00",
                                                                           max_date="2018-08-31 16:00",
                                                                           path_trios="data/2018R4_300025060010720.mat")

    ed_trios_incom[np.where(ed_trios_incom == 0)] = np.nan
    mask_wl = np.where((400 <= trios_wl) & (trios_wl <= 700))
    trios_wl = trios_wl[mask_wl]
    ed_trios_incom = ed_trios_incom[mask_wl]
    ed_trios_below = ed_trios_below[mask_wl]
    ...
    ed_dort, eu_dort, eo_dort = create_irradiance_data(zen_dort, azi_dort, rad_dort, ["0 cm",
                                                                                      ...
                                                                                      "180 cm",
                                                                                      "200 cm"])
    # Dort simulations n=1.355
    zen_dort_2, azi_dort_2, rad_dort_2 = open_radiance_data(path="data/dort-simulation-ri-355.h5")  # Path à changer
    ed_dort_2, eu_dort_2, eo_dort_2 = create_irradiance_data(zen_dort_2, azi_dort_2, rad_dort_2, ["0 cm",
                                                                                      "20 cm",
                                                                                      "40 cm",
                                                                                      "60 cm",
                                                                                      "80 cm",
    ...
    absorption_dort = np.zeros(len(z), dtype=([('r', 'f4'), ('g', 'f4'), ('b', 'f4')]))
    absorption_oden = absorption_dort.copy()

    absorption_dort["r"] = attenuation_coefficient((ed_dort["r"] - eu_dort["r"]), z) * ((ed_dort["r"] - eu_dort["r"]) / eo_dort["r"])
    absorption_dort["g"] = attenuation_coefficient((ed_dort["g"] - eu_dort["g"]), z) * ((ed_dort["g"] - eu_dort["g"]) / eo_dort["g"])
    absorption_dort["b"] = attenuation_coefficient((ed_dort["b"] - eu_dort["b"]), z) * ((ed_dort["b"] - eu_dort["b"]) / eo_dort["b"])

    absorption_oden["r"] = attenuation_coefficient((ed_oden["r"] - eu_oden["r"]), z) * ((ed_oden["r"] - eu_oden["r"]) / eo_oden["r"])
    absorption_oden["g"] = attenuation_coefficient((ed_oden["g"] - eu_oden["g"]), z) * ((ed_oden["g"] - eu_oden["g"]) / eo_oden["g"])
    absorption_oden["b"] = attenuation_coefficient((ed_oden["b"] - eu_oden["b"]), z) * ((ed_oden["b"] - eu_oden["b"]) / eo_oden["b"])
    ...
    # Transmittance published data
    wl_meiral, dt_a_meiral, a_meiral, dt_t_meiral, t_meiral = open_albedo_transmittance_meiralportal()
    ...
    ax2[0].plot(absorption_oden["r"], z, linewidth=0.9)
    ax2[0].plot(absorption_dort["r"], z, linewidth=0.9)

    ax2[1].plot(absorption_oden["g"], z, linewidth=0.9)
    ax2[1].plot(absorption_dort["g"], z, linewidth=0.9)

    ax2[2].plot(absorption_oden["b"], z, linewidth=0.9)
    ax2[2].plot(absorption_dort["b"], z, linewidth=0.9)

    ax2[0].invert_yaxis()
    ax2[0].set_xscale("log")
    ax2[0].set_ylabel("Depth [cm]")

    ax2[0].set_xlabel(r"$a$ [1/m]")
    ax2[1].set_xlabel(r"$a$ [1/m]")
    ax2[2].set_xlabel(r"$a$ [1/m]")
    ...
    # Incoming irradiance
    ax4[0].plot(trios_wl, ed_trios_incom.mean(axis=1), color="gray", label="TriOS")
    ax4[0].fill_between(trios_wl, ed_trios_incom.min(axis=1), ed_trios_incom.max(axis=1), color="gray", alpha=0.6)

    ax4[0].plot(np.array([603, 544, 484]), np.array([ed_oden[0]["r"], ed_oden[0]["g"], ed_oden[0]["b"]]), label="Cam", marker="o", markersize=4, markerfacecolor="none", markeredgecolor="k", color="k", linestyle="--")
    ax4[0].plot(np.array([603, 544, 484]), np.array([ed_dort[0]["r"], ed_dort[0]["g"], ed_dort[0]["b"]]),  label="DORT", marker="s", markersize=4, markerfacecolor="none", markeredgecolor="red", color="red", linestyle="-.")
    ...
    # Irradiance below
    ax4[1].plot(trios_wl, ed_trios_below.mean(axis=1), color="gray", label="TriOS")
    ax4[1].fill_between(trios_wl, ed_trios_below.min(axis=1), ed_trios_below.max(axis=1), color="gray", alpha=0.6)

    ax4[1].plot(np.array([603, 544, 484]), np.array([ed_oden[-1]["r"], ed_oden[-1]["g"], ed_oden[-1]["b"]]), label="Cam", marker="o", markersize=4, markerfacecolor="none", markeredgecolor="k", color="k", linestyle="--")
    ax4[1].plot(np.array([603, 544, 484]), np.array([ed_dort[-1]["r"], ed_dort[-1]["g"], ed_dort[-1]["b"]]),  label="DORT", marker="s", markersize=4, markerfacecolor="none", markeredgecolor="red", color="red", linestyle="-.")
    ...
    # Transmittance
    ax4[2].plot(trios_wl, tr_trios.mean(axis=1) * 100, color="gray", label="TriOS")
    ax4[2].fill_between(trios_wl, tr_trios.min(axis=1) * 100, tr_trios.max(axis=1) * 100, color="gray", alpha=0.6)

    ax4[2].plot(np.array([603, 544, 484]), tr_cam * 100, label="Cam", marker="o", markersize=4, markerfacecolor="none", markeredgecolor="k", color="k", linestyle="--")
    ax4[2].plot(np.array([603, 544, 484]), tr_dort * 100, label="DORT", marker="s", markersize=4, markerfacecolor="none", markeredgecolor="red", color="red", linestyle="-.")

    ax4[2].set_xlabel("Wavelength [nm]")
    ax4[2].set_ylabel("Transmittance [%]")
    ...
    fig_c, ax_c = graph_dort_vs_measurements(z, ((ed_oden, eu_oden, eo_oden), (rc.ed, rc.eu, rc.eo)),
                                             ("previous fit", "legendre fit"))
