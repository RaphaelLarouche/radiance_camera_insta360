...
def minimization(init, matlab_engine, measured_radiance_profile, trios):
    ...
    HE_simulation = SeaIceSimulation(run_title=root_name, root_name=root_name, mode='HE60DORT', wavelength_list=[602])
    HE_simulation.set_z_grid(z_max=3.0)
    for i, b in enumerate(b_init):
        top, bot = i * 0.20, (i + 1) * 0.20
        if i < 10:
            HE_simulation.add_layer(z1=top, z2=bot, abs={'484': a_init[i], '544': 0.0683, '602': a_init[i]}, scat=b, dpf=pf_ice[i]) # bb arg is not relevent since we use a discretized phase function in a file indepêdnant of depth (g=0.98)
        elif i == 10:
    ...
    for i, d_keys in enumerate(depth_keys_order):
        ...
        eu_he_r, ed_he_r, eo_he_r = HE_analyze.get_Eudos_at_depth(depth_list[i], 602) # blue
