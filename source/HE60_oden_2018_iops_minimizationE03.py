...
def minimization(b_init, matlab_engine, measured_radiance_profile, trios):
    ...
    for i, b in enumerate(b_init):
        if i < 10:
            ...
            HE_simulation.add_layer(z1=top, z2=bot, abs={'484': 0.0430, '544': 0.0683, '602': 0.12}, scat=b, dpf=pf_ice) # bb arg is not relevent since we use a discretized phase function in a file indepêdnant of depth (g=0.98)
        elif i == 10:
            HE_simulation.add_layer(z1=2.0, z2=3.01, abs={'484': 1.36e-2, '544': 5.11e-2, '602': 2.224e-1}, scat=b,
                                    dpf='dpf_OTHG_0_90.txt')
    HE_simulation.run_simulation(printoutput=False)
    ...
    bands = [480, 540, 600]
    depth_list = [0.20, 0.40, 0.60, 0.80, 1.00, 1.20, 1.41, 1.60, 1.80, 2.00]
    ...
    for i, d_keys in enumerate(depth_keys_order):
        ...
        eu_he_b, eu_he_b, eo_he_b = HE_analyze.get_Eudos_at_depth(depth_list[i], 480) # blue

        # Quad loss on Eo blue only
        rel_err_blue = 100*(eo_oden_b - eo_he_b)/ed_oden_b
