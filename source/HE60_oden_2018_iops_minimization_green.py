...
def minimization(init, matlab_engine, measured_radiance_profile, trios):
    ...
    for i, b in enumerate(b_init):
        ...
        if i < 10:
            HE_simulation.add_layer(z1=top, z2=bot, abs={'484': 0.012, '544': a_init[i], '602': 0.12}, scat=b, dpf=pf_ice) # bb arg is not relevent since we use a discretized phase function in a file indepêdnant of depth (g=0.98)
        elif i == 10:
    ...
    for i, d_keys in enumerate(depth_keys_order):
        ...
        eu_he_g, ed_he_g, eo_he_g = HE_analyze.get_Eudos_at_depth(depth_list[i], 540) # blue
...
if __name__ == "__main__":
    ...
    a_start = np.array([0.175, # 0-20 cm
                        0.175, # 20 - 40 cm
                       0.175, # 40 - 60 cm
                        0.115,  # 60 - 80 cm
                        0.115,  # 80 - 100 cm
