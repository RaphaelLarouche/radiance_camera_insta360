...
def minimization(init, matlab_engine, measured_radiance_profile, trios):
    ...
    for i, b in enumerate(b_init):
        ...
        if i < 10:
            HE_simulation.add_layer(z1=top, z2=bot, abs={'484': a_init[i], '544': 0.0683, '602': 0.12}, scat=b, dpf=pf_ice) # bb arg is not relevent since we use a discretized phase function in a file indepêdnant of depth (g=0.98)
        elif i == 10:
            HE_simulation.add_layer(z1=2.0, z2=3.01, abs={'484': 1.36e-2, '544': 5.11e-2, '602': 2.224e-1}, scat=b,
    ...
    for i, d_keys in enumerate(depth_keys_order):
        ...
        eu_he_b, ed_he_b, eo_he_b = HE_analyze.get_Eudos_at_depth(depth_list[i], 480) # blue
...
if __name__ == "__main__":
    ...
    a_start = np.array([0.055, # 0-20 cm
                        0.085, # 20 - 40 cm
                       0.095, # 40 - 60 cm
                        0.095,  # 60 - 80 cm
                        0.095,  # 80 - 100 cm
