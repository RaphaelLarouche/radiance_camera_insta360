...
def minimization(init, matlab_engine, measured_radiance_profile, trios):
    ...
    for i, b in enumerate(b_init):
        ...
        if i < 10:
            HE_simulation.add_layer(z1=top, z2=bot, abs={'484': a_init[i], '544': 0.0683, '602': a_init[i]}, scat=b, dpf=pf_ice) # bb arg is not relevent since we use a discretized phase function in a file indepêdnant of depth (g=0.98)
        elif i == 10:
            HE_simulation.add_layer(z1=2.0, z2=3.01, abs={'484': 1.36e-2, '544': 5.11e-2, '602': a_init[i]}, scat=b,
    ...
    for i, d_keys in enumerate(depth_keys_order):
        ...
        # eu_he_g, eu_he_g, eo_he_g = HE_analyze.get_Eudos_at_depth(depth_list[i], 540) # green
        eu_he_r, ed_he_r, eo_he_r = HE_analyze.get_Eudos_at_depth(depth_list[i], 600) # blue

        # Quad loss on Eo blue only
        rel_err_blue = 100*(eo_oden_r - eo_he_r)/eo_oden_r
        quad_loss[i] = rel_err_blue**2
        HE_eo.append(eo_he_r)
        HE_eu.append(eu_he_r)
        HE_ed.append(ed_he_r)
        Oden_eo.append(eo_oden_r)
        Oden_eu.append(eu_oden_r)
        Oden_ed.append(ed_oden_r)
    ...
    ax[0].semilogx(np.array(HE_eo)/1.355**2, depth_list, label="Eo HE")
    ax[0].semilogx(Oden_eo, depth_list, label="Eo Oden")
    ax[0].title.set_text('Eo')
    ax[1].semilogx(np.array(HE_eu)/1.355**2, depth_list, label="Eu HE")
    ax[1].semilogx(Oden_eu, depth_list, label="Eu Oden")
    ax[1].title.set_text('Eu')
    ax[2].semilogx(np.array(HE_ed)/1.355**2, depth_list, label="Ed HE")
...
if __name__ == "__main__":
    ...
    a_start = np.array([0.300, # 0-20 cm
                        0.300, # 20 - 40 cm
                       0.150, # 40 - 60 cm
                        0.140,  # 60 - 80 cm
                        0.105,  # 80 - 100 cm
    ...
    eng=None
    minimization(init=(a_start, b_start), matlab_engine=None, measured_radiance_profile=od, trios=None)
